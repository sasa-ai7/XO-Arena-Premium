...
class _HexGridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    const double hexSize = 22.0; // radius of each hex
    final double w = hexSize * 2;
    final double h = hexSize * sqrt3;
    final double halfH = h / 2;

    int col = 0;
    for (double x = -hexSize; x < size.width + hexSize; x += w * 0.75) {
      final bool offset = col.isOdd;
      for (double y = -hexSize; y < size.height + hexSize; y += h) {
        final double cy = offset ? y + halfH : y;
        _drawHex(canvas, paint, Offset(x, cy), hexSize);
      }
      col++;
    }
  }
